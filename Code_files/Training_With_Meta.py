...
for train_index, test_index in kf.split(X, Y):
    ...
    ##################################################################################
    # Doing the experiment with PCA to see how many number of compnents is good
    pca = PCA()
    ...
    print(
        f'Number of components for {cumulative_variance_threshold * 100}% cumulative variance: {num_components_cumulative}')
    ...
    svc = LinearSVC()
    bagging_classifier = BaggingClassifier(svc, n_estimators=5, random_state=42)
    multilabel_classifier_brute = OneVsRestClassifier(bagging_classifier, n_jobs=-1)
    ...
    best_C = grid_search.best_params_['estimator__base_estimator__C']
    print(f"Best C for linear case is: {best_C}")

    # Using the best C to train a LinearSVC model
    best_svc_linear = LinearSVC(C=best_C)
    bagging_classifier = BaggingClassifier(best_svc_linear, n_estimators=5, random_state=42)
    multilabel_classifier_linear = OneVsRestClassifier(bagging_classifier, n_jobs=-1)
    ...
    print(
        f"F1 score for R3 with micro averaging is with bagging (Linear case with C = {best_C}): {f_score_linear: .4f}")
    print(f"Accuracy for R3 is with bagging (Linear case with C = {best_C}): {accuracy_linear: .4f}")
    ...
    def evaluate(individual):
        ...
        multilabel_classifier = OneVsRestClassifier(bagging_classifier, n_jobs=-1)
    ...
    bagging_classifier = BaggingClassifier(best_model, n_estimators=5, random_state=42)

    # Make SVM a Multilabel classifier
    multilabel_classifier_gen = OneVsRestClassifier(bagging_classifier, n_jobs=-1)
    ...
    if f_score_gen > f_score_linear:
        ...
        multilabel_classifier_best = OneVsRestClassifier(bagging_classifier, n_jobs=-1)
        ...
        print(
            f"F1 score for R3 with micro averaging with meta learning is (Genetic is the chosen one): {f_score_best: .4f}")
        print(
            f"Accuracy for R3 with micro averaging with meta learning is (Genetic is the chosen one): {accuracy_best: .4f}")
    else:
        # Training on the whole training subset available in each fold of CV
        ...
        accuracy_best = accuracy_score(Y_test, predictions_best)
        print(
            f"F1 score for R3 with micro averaging with meta learning is (Linear is the chosen one): {f_score_best: .4f}")
        print(
            f"Accuracy for R3 with micro averaging with meta learning is (Linear is the chosen one): {accuracy_best: .4f}")
...
print(f"The mean of F1-score with meta learning model 5 fold CV is: {np.mean(f1_list):.4f}")
print(f"The standard deviation of F1-score with meta learning model with 5 fold CV is: {np.std(f1_list):.4f}")

print(f"The mean of accuracy with meta learning model with 5 fold CV is: {np.mean(accuracy_list):.4f}")
print(f"The standard deviation of accuracy with meta learning model with 5 fold CV is: {np.std(accuracy_list):.4f}")